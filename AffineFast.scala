...
class AffineFast(val xn:Int, val yn:Int , val eps:Float , val p1:Float , val bcCount:Int) extends Layer {
  ...
  def update() {
    ...
    val r_W = rW /:/ (1f-p2t)
    val r_b = rb /:/ (1f-p2t)
    W += -ep * s_W / (r_W.map(a=>math.sqrt(a).toFloat) + delta ) 
    //+ W.map(a=>math.abs(a).toFloat)
    b += -ep * s_b / (r_b.map(a=>math.sqrt(a).toFloat) + delta ) 
    //+ b.map(a=>math.abs(a).toFloat)
//    reset()
