...
class BNparam(val xn:Int , val ep:Float , val p1:Float)extends Layer{
  ...
  def update(){
    ...
    beta += -alpha * s_beta /:/ (r_beta.map(a=>math.sqrt(a).toFloat) + delta )
    gamma += -alpha * s_gamma /:/ (r_gamma.map(a=>math.sqrt(a).toFloat) + delta )
    reset()
  }
  def reset(){
    dbeta = DenseVector.zeros[T](xn)
